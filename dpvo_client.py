...
def load_video_frames(video_path, calib, K, stride=1, skip=0):
    ...
    while True:
        ...
        intrinsics = np.array([calib[0]*0.5, calib[1]*0.5, calib[2]*0.5, calib[3]*0.5])
            ...
            for t, image, intrinsics in frames:
                ...
                if response_data["type"] == "slam_result":
                    if t < 0:
                        ...
                        if cfg.save_ply:
                            save_ply(args.name, np.array(points), np.array(colors))
                        
                        if cfg.save_colmap:
                            save_output_for_COLMAP(args.name, PoseTrajectory3D(poses_se3=poses, timestamps=timestamps), 
                                                   np.array(points), np.array(colors), *intrinsics, H=image.shape[0], W=image.shape[1])
                        
                        if cfg.save_trajectory:
                            traj = PoseTrajectory3D(positions_xyz=np.array(poses)[:,:3], 
                                                    orientations_quat_wxyz=np.array(poses)[:, [6, 3, 4, 5]], 
                                                    timestamps=np.array(timestamps))
                            file_interface.write_tum_trajectory_file(f"saved_trajectories/{args.name}.txt", traj)
                        ...
                        if cfg.plot:
                            ...
                            plot_trajectory(traj, title=f"DPVO Trajectory Prediction for {args.name}", filename=f"trajectory_plots/{args.name}.pdf")
                    else:
                        frame_count += 1
