...
class DPVOStatelessSLAM:
    ...
    def _deserialize_state(self, state: Dict) -> DPVO:
        ...
        for attr in ['poses_', 'patches_', 'intrinsics_', 'points_', 'colors_', 
                    'index_', 'index_map_', 'net', 'ii', 'jj', 'kk',
                    'ii_inac', 'jj_inac', 'kk_inac', 'weight_inac', 'target_inac']:
        ...
        slam.pg.intrinsics_[:slam.n+1] = torch.from_numpy(state["pg_intrinsics"]).to(slam.device)
        slam.pg.poses_[:slam.n+1] = torch.from_numpy(state["pg_poses"]).to(slam.device)
        slam.pg.patches_[:slam.n+1] = torch.from_numpy(state["pg_patches"]).to(slam.device)
        slam.pg.colors_[:slam.n+1] = torch.from_numpy(state["pg_colors"]).to(slam.device)
        slam.pg.index_[:slam.n+1] = torch.from_numpy(state["pg_index"]).to(slam.device)
        slam.pg.index_map_[:slam.n+1] = torch.from_numpy(state["pg_index_map"]).to(slam.device)
    ...
    # ------------------------------------------------------------------
    @torch.no_grad()
    def run(self, frame: np.ndarray, intrinsics: np.ndarray, timestamp: float, 
            *, state: Optional[Dict] = None) -> Tuple[Dict, Dict]:
        """
            ...
            if state is None:
                ...
                return state, {
                    ...
                    "metrics": {
                        ...
                        "avg_fps": self._n_frames / self._total_compute_time if self._total_compute_time > 0 else 0,
        ...
        if state is None:
            slam = DPVO(self.config, self.network, ht=frame_tensor.shape[1], wd=frame_tensor.shape[2], viz=False)
...
class DPVOSLAMService:
    ...
    async def websocket_config(self, websocket: WebSocket):
        ...
        try:
            ...
            # TODO: This should ideally be done in a more structured way, can i just write (and call) a method?
            self.slam = DPVOStatelessSLAM(
                cfg=self.cfg,
            ...
            while True:
                ...
                if "type" in message and message["type"] == "end_stream":
                    ...
                    state, result = await asyncio.to_thread(
                        self.slam.run, None, intrinsics, -1, state=self.sessions.get(session_id)
                    )
                ...
                if "image" not in message:
                    await websocket.send_json({
                        "type": "error",
                        "message": "Missing 'image' field in message"
                    })
                    continue
                ...
                try:
                    ...
                    response = {
                        ...
                        "metrics": {
                            **result["metrics"],
                            "session_fps": frame_count / (time.time() - session_start_time),
